...
def _get_confidence(result: dict) -> dict:
    ...
    if row_count == 0:
        ...
        return {
            "completeness_note": f"仅 {row_count} 行数据",
            "confidence_label": CONFIDENCE_LOW,
            "confidence_note": "样本量过小，结论仅供参考",
        }
    elif missing_count > 0:
        return {
            "completeness_note": f"{row_count} 行，{len(columns)} 列（检测到缺失值）",
            "confidence_label": CONFIDENCE_MEDIUM,
            "confidence_note": "部分字段存在缺失值，结论有一定参考价值",
        }
    else:
        return {
            "completeness_note": f"{row_count} 行完整数据, {len(columns)} 列",
            "confidence_label": CONFIDENCE_HIGH,
            "confidence_note": f"基于 {row_count} 行完整数据，结果可靠",
        }
